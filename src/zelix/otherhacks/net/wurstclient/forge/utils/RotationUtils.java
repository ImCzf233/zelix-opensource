...
public final class RotationUtils
{
    ...
    public static double getAngleToLastReportedLookVec(final Vec3d vec) {
        ...
        try {
            final Field yawField = EntityPlayerSP.class.getDeclaredField(ForgeWurst.getForgeWurst().isObfuscated() ? "field_175164_bL" : "lastReportedYaw");
            yawField.setAccessible(true);
            lastReportedYaw = MathHelper.wrapDegrees(yawField.getFloat(player));
            final Field pitchField = EntityPlayerSP.class.getDeclaredField(ForgeWurst.getForgeWurst().isObfuscated() ? "field_175165_bM" : "lastReportedPitch");
    ...
    public static void faceVectorPacket(final Vec3d vec) {
        ...
        try {
            final Method onUpdateWalkingPlayer = pl.getClass().getDeclaredMethod(ForgeWurst.getForgeWurst().isObfuscated() ? "func_175161_p" : "onUpdateWalkingPlayer", (Class<?>[])new Class[0]);
