...
public final class KillauraHack extends Hack
{
    ...
    public void onUpdate(final WUpdateEvent event) {
        ...
        Stream<EntityLivingBase> stream = (Stream<EntityLivingBase>)world.loadedEntityList.parallelStream().filter(e -> e instanceof EntityLivingBase).map(e -> e).filter(e -> !e.isDead && e.getHealth() > 0.0f).filter(e -> player.getDistanceSq(e) <= rangeSq).filter(e -> e != player).filter(e -> !(e instanceof EntityFakePlayer));
        if (this.filterPlayers.isChecked()) {
            stream = stream.filter(e -> !(e instanceof EntityPlayer));
        }
        if (this.filterSleeping.isChecked()) {
            stream = stream.filter(e -> !(e instanceof EntityPlayer) || !e.isPlayerSleeping());
        }
        if (this.filterFlying.getValue() > 0.0) {
            final AxisAlignedBB box;
            final AxisAlignedBB box2;
            final World world2;
            stream = stream.filter(e -> {
                if (!(e instanceof EntityPlayer)) {
                    ...
                    box = e.getEntityBoundingBox();
                    box2 = box.union(box.offset(0.0, -this.filterFlying.getValue(), 0.0));
                    return world2.collidesWithAnyBlock(box2);
                }
            });
        }
        ...
        if (this.filterBabies.isChecked()) {
            stream = stream.filter(e -> !(e instanceof EntityAgeable) || !e.isChild());
        }
        if (this.filterPets.isChecked()) {
            stream = stream.filter(e -> !(e instanceof EntityTameable) || !e.isTamed()).filter(e -> !(e instanceof AbstractHorse) || !e.isTame());
        }
    ...
    private enum Priority
    {
        ...
        private Priority(final String name, final ToDoubleFunction<EntityLivingBase> keyExtractor) {
            this.name = name;
            this.comparator = Comparator.comparingDouble((ToDoubleFunction<? super EntityLivingBase>)keyExtractor);
        }
