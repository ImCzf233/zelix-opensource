...
public final class NukerHack extends Hack
{
    ...
    public void onUpdate(final WUpdateEvent event) {
        ...
        final List<BlockPos> blocks = stream.filter(pos -> eyesPos.squareDistanceTo(new Vec3d(pos)) <= rangeSq).filter(pos -> BlockUtils.canBeClicked(pos)).filter(this.mode.getSelected().getValidator(this)).sorted(Comparator.comparingDouble(pos -> eyesPos.squareDistanceTo(new Vec3d(pos)))).collect((Collector<? super BlockPos, ?, List<BlockPos>>)Collectors.toList());
