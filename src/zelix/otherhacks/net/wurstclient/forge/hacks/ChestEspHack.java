...
public final class ChestEspHack extends Hack
{
    ...
    public void onUpdate(final WUpdateEvent event) {
        ...
        for (final TileEntity tileEntity : world.loadedTileEntityList) {
            if (tileEntity instanceof TileEntityChest) {
                ...
                final BlockPos pos = chest.func_174877_v();
                AxisAlignedBB bb = BlockUtils.getBoundingBox(pos);
                if (chest.adjacentChestXNeg != null) {
                    final BlockPos pos2 = chest.adjacentChestXNeg.func_174877_v();
                    final AxisAlignedBB bb2 = BlockUtils.getBoundingBox(pos2);
                    bb = bb.union(bb2);
                }
                else if (chest.adjacentChestZNeg != null) {
                    final BlockPos pos2 = chest.adjacentChestZNeg.func_174877_v();
                    final AxisAlignedBB bb2 = BlockUtils.getBoundingBox(pos2);
                    bb = bb.union(bb2);
                }
                ...
                final BlockPos pos3 = ((TileEntityEnderChest)tileEntity).func_174877_v();
    ...
    public void onRenderWorldLast(final RenderWorldLastEvent event) {
        ...
        final ArrayList<AxisAlignedBB> list;
        this.minecarts.forEach(e -> {
            offsetX = -(e.posX - e.lastTickPosX) + (e.posX - e.lastTickPosX) * n;
            offsetY = -(e.posY - e.lastTickPosY) + (e.posY - e.lastTickPosY) * n;
            offsetZ = -(e.posZ - e.lastTickPosZ) + (e.posZ - e.lastTickPosZ) * n;
        ...
        if (this.style.getSelected().lines) {
            final Vec3d start = RotationUtils.getClientLookVec().add(0.0, (double)WMinecraft.getPlayer().getEyeHeight(), 0.0).add(TileEntityRendererDispatcher.staticPlayerX, TileEntityRendererDispatcher.staticPlayerY, TileEntityRendererDispatcher.staticPlayerZ);
            GL11.glBegin(1);
            GL11.glColor4f(0.0f, 1.0f, 0.0f, 0.5f);
