...
public final class RadarHack extends Hack
{
    ...
    public void onUpdate(final WUpdateEvent event) {
        ...
        this.entities.clear();
        final Entity entity;
        Stream<Entity> stream = (Stream<Entity>)world.loadedEntityList.parallelStream().filter(e -> !e.isDead && e != entity).filter(e -> !(e instanceof EntityFakePlayer)).filter(e -> e instanceof EntityLivingBase).filter(e -> e.getHealth() > 0.0f);
