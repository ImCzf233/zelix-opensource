...
public final class AutoFarmHack extends Hack
{
    ...
    public void onUpdate(final WUpdateEvent event) {
        ...
        final List<BlockPos> blocks = this.getBlockStream(eyesBlock, blockRange).filter(pos -> eyesVec.squareDistanceTo(new Vec3d(pos)) <= rangeSq).filter(pos -> BlockUtils.canBeClicked(pos)).collect((Collector<? super BlockPos, ?, List<BlockPos>>)Collectors.toList());
        this.registerPlants(blocks);
        final List<BlockPos> blocksToHarvest = blocks.parallelStream().filter((Predicate<? super Object>)this::shouldBeHarvested).sorted(Comparator.comparingDouble(pos -> eyesVec.squareDistanceTo(new Vec3d(pos)))).collect((Collector<? super Object, ?, List<BlockPos>>)Collectors.toList());
        final List<BlockPos> blocksToReplant = this.getBlockStream(eyesBlock, blockRange).filter(pos -> eyesVec.squareDistanceTo(new Vec3d(pos)) <= rangeSq).filter(pos -> BlockUtils.getMaterial(pos).isReplaceable()).filter(pos -> this.plants.containsKey(pos)).filter(this::canBeReplanted).sorted(Comparator.comparingDouble(pos -> eyesVec.squareDistanceTo(new Vec3d(pos)))).collect((Collector<? super BlockPos, ?, List<BlockPos>>)Collectors.toList());
