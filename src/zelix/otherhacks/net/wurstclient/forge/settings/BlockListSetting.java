...
public final class BlockListSetting extends Setting
{
    ...
    public void fromJson(final JsonElement json) {
        ...
        this.blockNames.clear();
        StreamSupport.stream((Spliterator<Object>)json.getAsJsonArray().spliterator(), true).filter(e -> e.isJsonPrimitive()).filter(e -> e.getAsJsonPrimitive().isString()).map(e -> Block.getBlockFromName(e.getAsString())).filter(Objects::nonNull).map(b -> BlockUtils.getName(b)).distinct().sorted().forEachOrdered(s -> this.blockNames.add(s));
    }
