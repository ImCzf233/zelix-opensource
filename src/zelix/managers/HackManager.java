...
public class HackManager
{
    ...
    public static ArrayList<Hack> pluginhacks;
    private GuiManager guiManager;
    private ClickGuiScreen guiScreen;
    
    public HackManager() {
        HackManager.hacks = new ArrayList<Hack>();
        HackManager.pluginhacks = new ArrayList<Hack>();
        addHack(new PingSpoof());
        addHack(new LOADTEST());
        addHack(new WTap());
        addHack(new DiedwithLightning());
    ...
    public static void addPluginHack(final Hack hack) {
        for (int i = 0; i < HackManager.hacks.size(); ++i) {
            if (HackManager.hacks.get(i).getName().equals(hack.getName())) {
                return;
            }
        }
        HackManager.hacks.add(hack);
    ...
    public static void unregist(final Hack hack) {
        for (int i = 0; i <= HackManager.pluginhacks.size(); ++i) {
            if (HackManager.pluginhacks.get(i).getName().equals(hack.getName())) {
                HackManager.pluginhacks.remove(i);
            }
        }
        for (int i = 0; i <= HackManager.hacks.size(); ++i) {
            if (HackManager.hacks.get(i).getName().equals(hack.getName())) {
                HackManager.hacks.remove(i);
            }
        }
    }
    ...
    public static void onCameraSetupS(final EntityViewRenderEvent.CameraSetup event) {
        for (final Hack hack : getHacks()) {
            if (hack.isToggled()) {
                hack.onCameraSetupS(event);
            }
        }
    }
