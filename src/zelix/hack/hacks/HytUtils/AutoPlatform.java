...
public class AutoPlatform extends Hack
{
    ...
    BooleanValue alsoActivateOnLand;
    
    public AutoPlatform() {
        super("AutoPlatform", HackCategory.HYT_UTILS);
        this.criticalSpeed = new NumberValue("CriticalSpeed", -1.0, -2.5, -0.5);
        this.alsoActivateOnLand = new BooleanValue("AlsoActivateOnLand", Boolean.valueOf(false));
        this.addValue(this.criticalSpeed, this.alsoActivateOnLand);
    }
    ...
    private boolean isBlockBelowAllVoid(final Wrapper wrapper) {
        if (this.alsoActivateOnLand.getValue()) {
            return true;
        }
