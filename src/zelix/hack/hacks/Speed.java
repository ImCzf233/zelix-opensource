...
public class Speed extends Hack
{
    ...
    public Speed() {
        ...
        this.mode = new ModeValue("Mode", new Mode[] { new Mode("Legit", true), new Mode("Basic", false), new Mode("OldHypixel", false), new Mode("HYT", false), new Mode("AAC4", false), new Mode("AAC", false), new Mode("VulcanLowHop", false) });
    ...
    public static int getSpeedEffect() {
        if (Wrapper.INSTANCE.player().isPotionActive(Potion.getPotionById(1))) {
            return Wrapper.INSTANCE.player().getActivePotionEffect(Potion.getPotionById(1)).getAmplifier() + 1;
        }
        return 0;
    }
    ...
    public static int getSpeedDuration() {
        if (Wrapper.INSTANCE.player().isPotionActive(Potion.getPotionById(1))) {
            return Wrapper.INSTANCE.player().getActivePotionEffect(Potion.getPotionById(1)).getDuration();
        }
        return 0;
    }
    ...
    public void onJump(final LivingEvent.LivingJumpEvent event) {
        if ((!this.mode.getMode("Legit").isToggled() && Wrapper.INSTANCE.player() == null) || Wrapper.INSTANCE.player().moveForward <= 0.0f || !Wrapper.INSTANCE.player().isSprinting() || getSpeedDuration() <= 20) {
            return;
        }
        if (event.getEntity() instanceof EntityPlayer && event.getEntity() == Wrapper.INSTANCE.player()) {
            if (getSpeedEffect() <= 0) {
                final EntityPlayerSP player;
                final EntityPlayerSP thePlayer = player = Wrapper.INSTANCE.player();
                player.motionX *= 1.0049999952316284;
                final EntityPlayerSP player2;
                final EntityPlayerSP thePlayer2 = player2 = Wrapper.INSTANCE.player();
                player2.motionZ *= 1.0049999952316284;
            }
        }
    }
