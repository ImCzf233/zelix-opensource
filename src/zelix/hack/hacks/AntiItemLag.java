            ...
            for (int i = 0; i < list2.size(); ++i) {
                ...
                if (targetList.size() > this.INT_ITEM_LAG_MAX) {
                    for (int k = this.INT_ITEM_LAG_DOWN_TO - 1; k < targetList.size(); ++k) {
                        ...
                        final Method method = ReflectionHelper.findMethod((Class<? super WorldClient>)World.class, worldClient, new String[] { "isChunkLoaded", "func_175680_a" }, Integer.TYPE, Integer.TYPE, Boolean.TYPE);
                        ...
                        if (entityIn.addedToChunk && isLoad) {
                            worldClient.getChunk(x, z).removeEntity(entityIn);
                        }
