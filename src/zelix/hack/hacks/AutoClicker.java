...
public class AutoClicker extends Hack
{
    ...
    public AutoClicker() {
        ...
        this.playerMouseInput = ReflectionHelper.findMethod((Class<? super Object>)GuiScreen.class, (Object)null, new String[] { "func_73864_a", "mouseClicked" }, Integer.TYPE, Integer.TYPE, Integer.TYPE);
        AutoClicker.mode = new ModeValue("Mode", new Mode[] { new Mode("Left", true), new Mode("Right", false), new Mode("Both", false) });
        AutoClicker.inventoryFill = new BooleanValue("InvFill", Boolean.valueOf(false));
        ...
        this.addValue(AutoClicker.mode, AutoClicker.inventoryFill, AutoClicker.CPS, AutoClicker.RandomMS, this.onlySword, this.BlockOnly);
    }
            ...
            if (AutoClicker.mode.getMode("Left").isToggled()) {
                if (Mouse.isButtonDown(0)) {
                    if (HackManager.getHack("Reach").isToggled()) {
                        ((Reach)HackManager.getHack("Reach")).doReach();
                    }
                    this.click();
                }
            }
            ...
            for (final EnumHand enumhand : EnumHand.values()) {
                ...
                if (Wrapper.INSTANCE.mc().objectMouseOver != null) {
                    switch (Wrapper.INSTANCE.mc().objectMouseOver.typeOfHit) {
                        ...
                        case BLOCK: {
                            final BlockPos blockpos = Wrapper.INSTANCE.mc().objectMouseOver.getBlockPos();
                            if (Wrapper.INSTANCE.mc().world.getBlockState(blockpos).func_185904_a() == Material.AIR) {
                                break;
                            }
