...
public class NoSlow extends Hack
{
    ...
    public void onInputUpdate(final InputUpdateEvent event) {
        Utils.nullCheck();
        if (!(Wrapper.INSTANCE.player().getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemSword)) {
            return;
        }
        ...
        if (this.mode.getMode("HYT 4V4").isToggled() && Mouse.isButtonDown(1) && this.mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemSword) {
            this.mc.getConnection().sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(new BlockPos(-1, -1, -1), Minecraft.getMinecraft().objectMouseOver.sideHit, EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
        }
    }
