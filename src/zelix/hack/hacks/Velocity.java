...
public class Velocity extends Hack
{
    ...
    public Velocity() {
        ...
        this.mode = new ModeValue("Mode", new Mode[] { new Mode("1231HYT", true), new Mode("Simple", false), new Mode("AAC4", false), new Mode("Tick", false) });
    ...
    public void onClientTick(final TickEvent.ClientTickEvent event) {
        if (this.mode.getMode("1231HYT").isToggled()) {
            final EntityPlayerSP player = Wrapper.INSTANCE.player();
            if (player.hurtTime > 1 && !player.isDead && player.hurtTime <= 6 && !Wrapper.INSTANCE.player().isInWater() && !this.mc.player.isPotionActive(MobEffects.SPEED)) {
            ...
            if (!Wrapper.INSTANCE.player().onGround) {
                if (this.ishurt) {
                    final Field field = ReflectionHelper.findField((Class)EntityPlayer.class, new String[] { "speedInAir", "field_71102_ce" });
                    try {
