...
public class AutoMine extends Hack
{
    ...
    public void onClientTick(final TickEvent.ClientTickEvent event) {
        if (AutoMine.start && AutoMine.PathReady) {
            ...
            if (Wrapper.INSTANCE.mc().player.motionX == 0.0 && (Wrapper.INSTANCE.mc().player.motionY == -0.1552320045166016 || Wrapper.INSTANCE.mc().player.motionY == -0.0784000015258789) && Wrapper.INSTANCE.mc().player.motionZ == 0.0) {
                if (this.timeHelper2.hasReached(10000L)) {
                    new Thread() {
                        @Override
                        public void run() {
                            try {
            ...
            if (!this.timeHelper4.hasReached(AutoMine.delay.getValue())) {
                return;
            }
            ...
            if (vec3.squareDistanceTo(player) < 0.4) {
                ...
                else if (this.timeHelper.hasReached(800L)) {
                    AutoMine.keyDown = "W";
                }
                ...
                faceEntity(vec3.getX(), vec3.getY() - 1.0, vec3.getZ());
                if (BlockUtils.getNeaestPlayerBlockDistance(pos.getX(), pos.getY(), pos.getZ()) > 2.0f) {
                    if (this.timeHelper3.hasReached(1000L)) {
                        AutoMine.keyDown = "W";
                    }
                }
                else {
    ...
    public void placeBlock() {
        ...
        for (int length = (values = EnumFacing.values()).length, j = 0; j < length; ++j) {
            ...
            if (eyesPos.squareDistanceTo(new Vec3d((Vec3i)blockDown).add(0.5, 0.5, 0.5)) < eyesPos.squareDistanceTo(new Vec3d((Vec3i)neighbor).add(0.5, 0.5, 0.5)) && BlockUtils.getBlock(neighbor).canCollideCheck(Wrapper.INSTANCE.mc().world.getBlockState(neighbor), false)) {
                ...
                if (eyesPos.squareDistanceTo(hitVec) <= 18.0625) {
                    ...
                    for (final String fieldName : new String[] { "field_78781_i", "blockHitDelay" }) {
                        try {
