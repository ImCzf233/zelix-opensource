...
public class RenderUtil implements Utils
{
    ...
    public static void drawBorderedRect(final float x, final float y, final float width, final float height, final float outlineThickness, final int rectColor, final int outlineColor) {
        ...
        GL11.glEnable(2848);
        final float cornerValue;
        final float n;
        GLUtil.setup2DRendering(() -> {
            color(outlineColor);
            GL11.glLineWidth(outlineThickness);
            cornerValue = (float)(outlineThickness * 0.19);
            GLUtil.render(1, () -> {
                GL11.glVertex2d((double)x, (double)(y - n));
                GL11.glVertex2d((double)x, (double)(y + height + n));
                GL11.glVertex2d((double)(x + width), (double)(y + height + n));
                GL11.glVertex2d((double)(x + width), (double)(y - n));
