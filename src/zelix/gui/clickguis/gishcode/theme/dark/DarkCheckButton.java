    ...
    public void drawComponent(final Component component, final int mouseX, final int mouseY) {
        ...
        for (final Mode mode : button.getModeValue().getModes()) {
            if (mode.getName().equals(text)) {
                this.theme.fontRenderer.drawString("- " + text, button.getX() + 5, MathUtils.getMiddle(button.getY(), button.getY() + button.getDimension().height) - this.theme.fontRenderer.FONT_HEIGHT / 3 - 1, mode.isToggled() ? mainColorInv : strColor);
            }
        }
    }
