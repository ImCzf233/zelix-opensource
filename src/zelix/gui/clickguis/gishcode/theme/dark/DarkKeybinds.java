...
public class DarkKeybinds extends ComponentRenderer
{
    ...
    public void drawComponent(final Component component, final int mouseX, final int mouseY) {
        ...
        if (keybind.getMod().getKey() == -1) {
            ...
            this.theme.fontRenderer.drawString(keybind.isEditing() ? "|" : Keyboard.getKeyName(keybind.getMod().getKey()), keybind.getX() + keybind.getDimension().width / 2 + nameWidth / 2 - this.theme.fontRenderer.getStringWidth(Keyboard.getKeyName(keybind.getMod().getKey())) / 2, keybind.getY() + 2, keybind.isEditing() ? mainColorInv : mainColorInv);
        }
    }
